"""SBDS 居住者登録API — ConsentFlowProcessor を使った同意取得フロー

POST /api/v1/residents/register
PORT = 8091
"""
...
class ResidentsHandler(BaseHTTPRequestHandler):
    """HTTP request handler for SBDS Residents Registration API."""
    ...
    def do_GET(self) -> None:
        ...
        if parsed.path == '/health':
            self._send_json(200, {'status': 'ok', 'module': 'residents', 'version': '1.0'})
        else:
            self._send_json(404, {'error': 'Not Found'})

    def do_POST(self) -> None:
        parsed = urlparse(self.path)
        if parsed.path == '/api/v1/residents/register':
    ...
    def _handle_register(self) -> None:
        ...
        if result.success:
            self._send_json(200, {
                ...
                'gdpr_notification': gdpr_applicable,
            })
        else:
            self._send_json(400, {
                'success': False,
...
def run(port: int = PORT) -> None:  # pragma: no cover
    server = HTTPServer(('0.0.0.0', port), ResidentsHandler)  # nosec B104 — Cloud Run requires 0.0.0.0 binding
